...
class WorkerThread(threading.Thread):
    ...
    def __init__(self, tags, threadName ) :
        '''Initialization'''
        threading.Thread.__init__(self)
        self.tags = tags
        self.threadName = threadName
        self.csvReader = CsvReader(threadName+'.csv')
        self.existPages = self.csvReader.getOffset()
            

    def getExistPages(self):
        return self.existPages

    def filter(self, candidate):
        prev = len(self.existPages)
        self.existPages.add(candidate)
        current = len(self.existPages)
        return prev == current
    ...
    def parsePage(self, writer, obj, debug):
        ...
        for item in items:
            try:
                ...
                exists = self.filter(offset)
                
                if exists :
                    print(self.threadName+'Duplicace item...')
                    continue
    ...
    #    tags = findAllTag(baseUrl, offset, debug)            
        ...
        csvFile = open(self.threadName+".csv", "a+", encoding='utf-8')
        try:
